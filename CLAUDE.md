...
- **Timer types**: 3 types: `TIMER_ONCE_MANUAL` (default — channel + time), `TIMER_SERIES`, `TIMER_ONCE_CREATED_BY_SERIES` (read-only child). EPG-driven one-shot creation falls back to the manual type with channel/time prefilled from the EPG entry.
